...
import { ArrowLeft, Star, User, Image } from 'lucide-react';
...
export default function SkillsPage() {
  ...
  const { data: skills, refetch, isLoading: loadingSkills } = useQuery({
    queryKey: ['skills', selectedStudentId],
    queryFn: async () => {
      const { data } = await supabase
        .from('skills')
        .select('id, skill_name, note, image_url, created_at')
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 -mx-4 px-4 md:mx-0 md:px-0">
      ...
      <div className="flex items-center gap-3 py-4">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="text-xl font-bold truncate">Skills</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 truncate">Track & record skills</p>
        </div>
      </div>
      ...
      {selectedStudentId ? (
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4">
            <h3 className="font-bold text-gray-900 dark:text-white mb-4">Record Skill</h3>
            <AddSkillForm studentId={selectedStudentId} onSuccess={refetch} />
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4">
            ...
            {loadingSkills ? (
              ...
              <div className="space-y-3 max-h-[50vh] overflow-y-auto -mx-2 px-2">
                {skills.map(skill => (
                  <div key={skill.id} className="flex gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    {skill.image_url ? (
                      <img src={skill.image_url} alt="Skill" className="w-14 h-14 object-cover rounded-lg flex-shrink-0" />
                    ) : (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-8 text-center">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Choose above to record or view skills</p>
        </div>
      )}
    </div>
  );
